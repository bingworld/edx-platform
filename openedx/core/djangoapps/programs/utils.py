...
class ProgramDataExtender(object):
    ...
    def _execute(self, prefix, *args):
        ...
        [getattr(self, handler)(*args) for handler in self._handlers(prefix)]
    ...
    def _attach_run_mode_is_enrollment_open(self, run_mode):
        enrollment_start = self.course_overview.enrollment_start or DEFAULT_ENROLLMENT_START_DATE
        enrollment_end = self.course_overview.enrollment_end or datetime.datetime.max.replace(tzinfo=pytz.UTC)
        run_mode['is_enrollment_open'] = enrollment_start <= timezone.now() < enrollment_end
